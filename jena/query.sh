...
#echo 'drop table pand10; select gml_id, st_transform(pand.wkb_geometry,4326) as geometry, pand.wkb_geometry, st_area(pand.wkb_geometry) as area into pand10 from pand where st_intersects(st_transform(pand.wkb_geometry,4326), ST_MakeEnvelope(5, 52, 5.2, 52.2, 4326))' | psql -A -t bgt
...
echo '\\timing on
select count(*) from pand10 where area > 1000;' | psql -A -t bgt
...
time curl "http://localhost:3030/ds?query=$(jq -sRr @uri scan.rq)" | fgrep float | wc
